...
export class ShlyapaMarkupUtil {
  ...
  public parse(markup: string): Parsed {
    ...
    const m = body[1].match(regex).map((el) => {
      const res: ShlyapaMarkup = { item: null, sum: null, composition: null };
      //Cut ( ) if exists
      if (el[0] == '(') {
  ...
  private getOperand(el: string, item: string): Operand {
    const value = parseInt(item.match(ShlyapaMarkupUtil.parse_var_id)[0]);
    let sign = 1;
    const type = item[0] == '$' ? OperandType.VARIABLE : OperandType.CONST;
    if (el[0] == '-') sign = -1;
    return {
      value,
      sign,
