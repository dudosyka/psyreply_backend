...
export class UserProvider extends BaseProvider<UserModel> {
  ...
  async genChat(botModel: BotModel, user: User, chatId: number): Promise<UserModel> {
  ...
  async appendMessage(messageModel: MessageModel, user_id: number, isAdmin: number | null = null) {
    let isPropagate = true;
    if (!TransactionUtil.isSet()) {
      isPropagate = false;
      TransactionUtil.setHost(await this.sequelize.transaction())
    ...
    if (isAdmin)
      onCreate.recipient_id = isAdmin;

    await UserMessageModel.create(onCreate, TransactionUtil.getHost()).catch(err => {
      if (!isPropagate)
        TransactionUtil.rollback();
      throw err;
    });
