...
export class BlockController {
  ...
  @Get('assign/:jetBotId')
  async assignUserToBlockToken(
    @Req() req,
    @Param('jetBotId') jetBotId: number,
  ): Promise<{ link: string; linkdb: string }> {
    const hash = await this.blockProvider.createPassLink(
      req.user.blockId,
      req.user.week,
      jetBotId,
      req.user.companyId,
    ...
    return {
      link: 'https://client.psyreply.com/test/' + hash,
      linkdb: 'https://client.psyreply.com/results/' + dashHash,
    };
  }
